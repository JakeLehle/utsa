...
library(reticulate)
#virtualenv_create("r-reticulate") # /home/jake/.local/share/r-miniconda/envs/r-reticulate
use_virtualenv("r-reticulate")

```
``` {python Install and call in packages}
import scanpy as sc
import numpy as np
import scipy as sp
import pandas as pd
...
``` {r Bring in R packages}
BiocManager::install("scater")
BiocManager::install("scran")
install.packages("RColorBrewer")
BiocManager::install("slingshot")
BiocManager::install("monocle") #work on this
...
```
```{python Data}
sample_strings = ['Duo_M1', 'Duo_M2', 'Jej_M1', 'Jej_M2', 'Il_M1', 'Il_M2'] # Different sample groups.
sample_id_strings = ['3', '4', '5', '6', '7', '8'] # Last number in the sample id that is unique all other id numbers are the same. Wonder if this would work with more than 10 groups.
file_base = '/home/jake/Documents/SC_RNA_SEQ/data/Haber-et-al_mouse-intestinal-epithelium/GSE92332_RAW/GSM283657' #Path to the files. Look how the file ends with the id beginning of the file and the only thing missing is the sample id value that is unique for each sample.
exp_string = '_Regional_'
#different file types tsv = files with feature and barcode sequences corresponding to row and column indicies respectively. mtx
...
#Parse Filenames
sample = sample_strings.pop(0) # The .pop function pops out the first file in the list so you can load in one file first before looping in all o the other files and merging them to this starting object.
...
print('Out of loop')    
``` #Go to the console and hit an extra E
...
# Checking the total size of the data set the first number is the number of single cells in the data set the second is the number of deferentially expressed genes.
adata.shape
...
sc.pp.neighbors(adata_pp, use_rep='X') #changed from sc.pp.neighbors(adata_pp)
...
size_factors = computeSumFactors(py$data_mat, clusters=py$input_groups, min.mean=0.1)

size_factors = (f"sizeFactors(computeSumFactors(SingleCellExperiment(list(counts=data_mat)), clusters = input_groups, min.mean = {min_mean}))")
```
...
adata.obs['r.size_factors'] = size_factors
