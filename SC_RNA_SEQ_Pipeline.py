...
#%%
#R -i adata_ent
robjects.globalenv['adata_ent'] = adata[np.isin(adata.obs['louvain_final'], clusters_to_include),:].copy()
#Plot 1
colour_map = robjects.r(f'brewer.pal(20,"Set1")')
robjects.r(f'par(xpd=TRUE)')
robjects.r(f'par(mar=c(4.5,5.5,2,7))')
robjects.r(f'plot(reducedDims(adata_ent)$PCA[,1], reducedDims(adata_ent)$PCA[,2], col=colour_map[colData(adata_ent)$louvain_final], bty='L', xlab='PC1', ylab='PC2')')
robjects.r(f'legend(x=12, y=12, legend=unique(colData(adata_ent)$louvain_final), fill=colour_map[as.integer(unique(colData(adata_ent)$louvain_final))])')
