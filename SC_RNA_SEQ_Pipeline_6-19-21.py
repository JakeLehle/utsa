...
with localconverter(anndata2ri.converter):
    adata_ent = r('as(adata_ent, "SingleCellExperiment")')

import anndata2ri
from rpy2.robjects import r
anndata2ri.activate()

adata = r('as('adata_ent', SingleCellExperiment)')
#6/20/21
adata_ent.obs.dtypes
adata_ent.var.dtypes
adata_ent.write(filename = 'adata_ent.h5ad')
adata_ent.var['vst_variable'] = adata_ent.var['vst_variable'].astype(int)
robjects.globalenv["adata_ent"] = zellkonverter.AnnData2SCE(adata_ent, X_name = None, skip_assays = True, hdf5_backed = False)




#///////////////////////////////////
adata_ent.X = sparse.csc_matrix(adata_ent.X)
adata_ent_ent = adata_ent.X.astype(int)

print(adata_ent.X)
robjects.globalenv["adata_ent"] = zellkonverter.AnnData2SCE(adata)
robjects.globalenv['adata_ent'] = adata_ent
type(adata_ent)
from rpy2.robjects.packages import importr
